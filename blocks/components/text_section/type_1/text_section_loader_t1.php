<?php
/**
 * Text Section Component Loader
 */

class TextSectionLoaderT1 {
    ...
    private function generateShell($id, $navConfig, $componentMetadata) {
        $template = file_get_contents(__DIR__ . '/text_section_structure_t1.html');
        
        $navConfigJson = htmlspecialchars(json_encode($navConfig), ENT_QUOTES, 'UTF-8');
        $metadataJson = htmlspecialchars(json_encode($componentMetadata), ENT_QUOTES, 'UTF-8');
        
        $template = str_replace(
            '<section class="text-section-component" data-nav-handler="handleTextSectionNavigation">',
            '<section class="text-section-component" id="' . htmlspecialchars($id, ENT_QUOTES, 'UTF-8') . '" 
                 data-nav-handler="handleTextSectionNavigation"
                 data-nav-config=\'' . $navConfigJson . '\'
                 data-dynamic="true"
                 data-load-state="not-loaded"
                 data-init-hook="initializeTextSection"
    ...
    private function injectDataScript($id, $data) {
        ...
        $script .= 'if (typeof window.setTextSectionData === "function") {';
        $script .= '    window.setTextSectionData("' . htmlspecialchars($id, ENT_QUOTES, 'UTF-8') . '", data);';
        $script .= '} else {';
