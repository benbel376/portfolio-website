<?php
/**
 * Stats Grid Component Loader
 */

class StatsGridLoaderT1 {
    ...
    private function generateShell($id, $navConfig, $componentMetadata) {
        $template = file_get_contents(__DIR__ . '/stats_grid_structure_t1.html');
    ...
    private function injectDataScript($id, $data) {
        ...
        $script .= 'if (typeof window.setStatsGridData === "function") {';
        $script .= '    window.setStatsGridData("' . htmlspecialchars($id, ENT_QUOTES, 'UTF-8') . '", data);';
        $script .= '} else {';
