<?php
/**
 * Tech Stack Component Loader
 */

class TechStackLoaderT1 {
    ...
    private function generateShell($id, $navConfig, $componentMetadata) {
        $template = file_get_contents(__DIR__ . '/tech_stack_structure_t1.html');
    ...
    private function injectDataScript($id, $data) {
        ...
        $script .= 'if (typeof window.setTechStackData === "function") {';
        $script .= '    window.setTechStackData("' . htmlspecialchars($id, ENT_QUOTES, 'UTF-8') . '", data);';
        $script .= '} else {';
