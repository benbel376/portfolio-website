<?php
/**
 * Section Title Component Loader
 */

class SectionTitleLoaderT1 {
    ...
    private function generateShell($id, $navConfig, $componentMetadata) {
        $template = file_get_contents(__DIR__ . '/section_title_structure_t1.html');
    ...
    private function injectDataScript($id, $data) {
        ...
        $script .= 'if (typeof window.setSectionTitleData === "function") {';
        $script .= '    window.setSectionTitleData("' . htmlspecialchars($id, ENT_QUOTES, 'UTF-8') . '", data);';
        $script .= '} else {';
